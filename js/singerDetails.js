  ...
  function cli(){
	...
	function getSingerName(){
		...
		var singerName = decodeURI(location.search).split("=")[1];
        return singerName;
	}
	function loadThisSinger(){
		 $('.singerName').text(getSingerName());
		$.get(
		'getSingerInfo.do',
		{"singerName":getSingerName()},
		function(singerList){
		       $('.myImg>img').attr('src','../icon/'+singerList.icon);
		       $('#one>td').eq(0).text('姓名：'+singerList.name);    
		       $('#one>td').eq(1).text('性别：'+singerList.sex);    
		       $('#two>td').eq(0).text('别名：'+singerList.nickname);    
		       $('#two>td').eq(1).text('生日：'+singerList.birth);    
		       $('#three>td').eq(0).text('地区：'+singerList.classifyName);    
		       $('#three>td').eq(1).text('个人简介:'+singerList.introduce); 
		},
		'json'
