...
/*删除当前行*/
function deleThisRow(){
    $('.deleimg').bind('click', function () {
        //删除当前行
    	var $this = $(this);
    	//删除数据
    	var songName = $(this).parent().parent().parent().children()[1].innerText;
    	$.get(
    	'deleMyThisSong.do',
    	{'songName':songName},
    	function(val){
...
/*点击播放当前行*/
function playThisRow(){
	$('.playbo').bind('click', function () {
        /*player.src = c.source;*/
		player.src = $(this).parent().parent().children()[0].value;
        player.play();
...
//play
function play() {
    player.src = data[currentIndex].res;
    player.play();
...
function getSongName() {
	var songName = decodeURI(location.search).split("=")[1];
		    ...
		    function(songlist){
		    	  for(var i=0;i<songlist.length;i++){
				        if(songName == songlist[i].name){
				            ...
				            $($('#myplayer')[0].contentWindow.document.querySelector('.artist')).text(currentsonglist.singerName);
